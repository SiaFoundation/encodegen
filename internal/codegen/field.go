...
func NewField(owner *Struct, field *toolbox.FieldInfo, fieldType *toolbox.TypeInfo) (*Field, error) {
	...
	if isPrimitiveString(encodingMethod) {
		result.EncodingMethod = supportedPrimitives[encodingMethod].WriteFunction
		result.DecodingMethod = supportedPrimitives[encodingMethod].ReadFunction
		result.PrimitiveWriteCast = supportedPrimitives[encodingMethod].WriteCast
	} else {
