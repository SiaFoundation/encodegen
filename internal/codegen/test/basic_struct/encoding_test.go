...
func TestInvalidMessage(t *testing.T) {
	replaceByteConstants := []byte{0, 1, 2, 3, 4, 5, 6, 7, 255}
	// replace every byte with a constant and compare unmarshalled result
	for _, replaceByteConstant := range replaceByteConstants {
		buffer := &encodegen.ObjBuffer{}
		msg.MarshalBuffer(buffer)

		modifiedBytes := buffer.Bytes()
		for i := 0; i < len(modifiedBytes); i++ {
			modifiedBytes[i] = replaceByteConstant
		}
		modifiedBuffer := &encodegen.ObjBuffer{}
		modifiedBuffer.Write(modifiedBytes)

		newMessageUnofficial := Message{}
		(*Message)(&newMessageUnofficial).UnmarshalBuffer(modifiedBuffer)

		newMessageOfficial := Message{}
		encoding.Unmarshal(modifiedBuffer.Bytes(), &newMessageOfficial)

		assert.Equal(t, newMessageOfficial, newMessageUnofficial, "Unmarshaled struct does not equal one generated by reflection")
	}

	// replace every other byte - currently fails due to lack of bounds checking on slices
	// for _, replaceByteConstant := range replaceByteConstants {
	// 	buffer := &encodegen.ObjBuffer{}
	// 	msg.MarshalBuffer(buffer)

	// 	modifiedBytes := buffer.Bytes()
	// 	for i := 0; i < len(modifiedBytes)-1; i += 2 {
	// 		modifiedBytes[i] = replaceByteConstant
	// 	}
	// 	modifiedBuffer := &encodegen.ObjBuffer{}
	// 	modifiedBuffer.Write(modifiedBytes)
